...
import { setActiveWhatsAppProvider, deleteWhatsAppProvider } from "@/services/settingsService"
import { useAuth } from "@/components/auth-provider"

export function WhatsAppProviderList({ providers, onEdit, onDelete, onSetActive }) {
  const { toast } = useToast()
  const { token } = useAuth()

  const handleSetActive = async (id) => {
    try {
      await setActiveWhatsAppProvider(id, token)
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this provider?")) {
      try {
        await deleteWhatsAppProvider(id, token)
  ...
  if (providers.length === 0) {
    return (
      <div className="text-center py-12 border-2 border-dashed border-zinc-800 rounded-lg">
        <h3 className="text-lg font-medium text-zinc-300">No WhatsApp Providers Configured</h3>
