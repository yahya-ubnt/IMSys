...
import { Save, Wifi, Lock, User, ChevronRight, ChevronLeft, Loader2, Plus } from "lucide-react";
import { format } from "date-fns";
import { motion, AnimatePresence } from "framer-motion";
import { createBuilding, type Building, type Device } from "@/lib/deviceService";
import { mikrotikUserFormSchema, type MikrotikUserFormSchema } from "@/lib/validators";
...
interface MikrotikUserFormProps {
  isEditMode: boolean;
  initialData?: Partial<MikrotikUserFormData>;
  onSubmit: (data: Partial<MikrotikUserFormData>) => Promise<void>;
...
export function MikrotikUserForm({ isEditMode, initialData, onSubmit, routers, packages, buildings, stations, onBuildingsUpdate, onCancelGracePeriod, isSubmitting }: MikrotikUserFormProps) {
    ...
    useEffect(() => {
        if (mikrotikRouterId && serviceType) {
            ...
            if (!isEditMode) setPackageId(""); // Reset package only when creating
        } else {
            setFilteredPackages([]);
        }
    ...
    const validateStep = (step: number) => {
        ...
        let schema;
        if (step === 1) {
            schema = mikrotikUserFormSchema.pick({ mikrotikRouter: true, serviceType: true });
        } else if (step === 2) {
            schema = mikrotikUserFormSchema.pick({ package: true });
        } else {
            schema = mikrotikUserFormSchema;
        }
    ...
    const handleCreateBuilding = async () => {
        ...
        try {
            ...
            onBuildingsUpdate([...buildings, newBuilding]);
    ...
    const getFormData = (): Partial<MikrotikUserFormData> => ({
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="space-y-2 sm:col-span-2">
                                            ...
                                            <Select onValueChange={setPackageId} value={packageId} disabled={!mikrotikRouterId || !serviceType || filteredPackages.length === 0}>
                                                ...
                                                <SelectContent className="bg-zinc-800 text-white border-zinc-700">{filteredPackages.map(p => <SelectItem key={p._id} value={p._id} className="text-sm">{p.name} (KES {p.price})</SelectItem>)}</SelectContent>
                                            </Select>
                                ...
                                <motion.div key={3} custom={direction} variants={formVariants} initial="hidden" animate="visible" exit="exit" className="space-y-5">
                                    ...
                                    <div className="space-y-3 pt-4 border-t border-zinc-800">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            ...
                                            {serviceType === 'static' && <div className="space-y-2"><Label className="text-xs">MAC Address <span className="text-red-500">*</span></Label><Input value={macAddress} onChange={e => setMacAddress(e.target.value)} required className="h-9 bg-zinc-800 border-zinc-700 text-sm" placeholder="e.g., AA:BB:CC:DD:EE:FF" />{errors.macAddress && <p className="text-red-500 text-xs mt-1">{errors.macAddress}</p>}</div>}
