...
export function MpesaPaybillForm() {
    ...
    const fetchSettings = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setSettings(data.mpesaPaybill);
          if (data.mpesaPaybill) {
            reset(data.mpesaPaybill);
          }
        }
      } catch (error) {
  ...
  const onSubmit = async (data: PaybillFormValues) => {
    const promise = fetch('/api/settings/mpesa', {
      ...
      body: JSON.stringify({ type: 'paybill', data }),
    });

    toast.promise(promise, {
      loading: 'Updating Paybill settings...',
      success: (res) => {
        promise.then(res => res.json()).then(data => {
            setSettings(data.mpesaPaybill);
            reset(data.mpesaPaybill);
        });
        return 'Paybill settings updated successfully!';
      },
  ...
  const handleActivate = async () => {
    ...
    const promise = fetch('/api/settings/mpesa/activate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'paybill' }),
    });

    toast.promise(promise, {
        loading: 'Activating M-Pesa Paybill...',
    ...
    try {
        await promise;
        // Optionally refetch settings to update activation status
    } finally {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      <div className="bg-zinc-900/50 backdrop-blur-lg border border-zinc-700/50 rounded-xl">
        <div className="p-6">
          <h3 className="text-2xl font-bold text-white mb-2">M-Pesa Pay Bill Settings</h3>
          <p className="text-zinc-400">Configure your M-Pesa Pay Bill credentials and callback URLs.</p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-6">
                <div>
                  <Label htmlFor="paybillNumber" className="text-zinc-400">Pay Bill Number</Label>
                  <Controller name="paybillNumber" control={control} render={({ field }) => <Input {...field} className="bg-zinc-800 border-zinc-700 text-white" />} />
                  {errors.paybillNumber && <p className="text-red-500 text-sm mt-1">{errors.paybillNumber.message}</p>}
                </div>
                <div>
