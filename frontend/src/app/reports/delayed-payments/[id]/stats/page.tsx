...
import { ArrowUp, ArrowDown, Percent, Calendar, Hash, DollarSign } from 'lucide-react';
...
const StatCard = ({ title, value, icon: Icon, color, suffix = '' }: any) => (
    <motion.div
        ...
        className="bg-zinc-800/50 p-4 rounded-lg flex items-center gap-4 border border-zinc-700"
    >
    ...
    return (
        <div className="flex flex-col min-h-screen bg-zinc-900 text-white">
            ...
            <div className="flex-1 p-6 space-y-6">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-teal-500">Payment Insights</h1>
                    <p className="text-sm text-zinc-400">Financial history for {stats.name}.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <StatCard title="On-Time Rate" value={stats.onTimePaymentPercentage.toFixed(1)} icon={Percent} color="text-blue-400" suffix="%" />
                    <StatCard title="Avg. Delay" value={`${stats.averagePaymentDelay.toFixed(1)} days`} icon={Calendar} color="text-yellow-400" />
                    <StatCard title="Total Payments" value={stats.totalPayments} icon={Hash} color="text-purple-400" />
                    <StatCard title="Lifetime Value" value={`KES ${stats.lifetimeValue.toLocaleString()}`} icon={DollarSign} color="text-pink-400" />
                </div>
