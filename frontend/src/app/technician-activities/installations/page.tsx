...
export default function InstallationsPage() {
  ...
  const fetchActivities = useCallback(async () => {
    setLoading(true);
    try {
      const filters: { activityType: "Installation"; technicianId?: string } = { activityType: 'Installation' };
      ...
      setError((err instanceof Error) ? err.message : 'Failed to fetch installation activities.');
      toast({
        title: 'Error',
        description: (err instanceof Error) ? err.message : 'Failed to fetch installation activities. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-2 text-zinc-400">Loading installation activities...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-950 to-black text-white">
      ...
      <div className="flex-1 p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-blue-400">Installations</h1>
            <p className="text-zinc-400">View and manage technician installation activities.</p>
          </div>
          <Button onClick={() => router.push('/technician-activities/installations/new')} className="bg-blue-600 hover:bg-blue-700 text-white shadow-lg transition-all duration-300 hover:scale-105">
        ...
        <Card className="bg-zinc-900 border border-zinc-800 shadow-xl rounded-lg">
          <CardHeader>
            <CardTitle className="text-blue-400">Installation Activities Overview</CardTitle>
          </CardHeader>
          <CardContent>
            {/* Filters can go here */}
            <DataTable
